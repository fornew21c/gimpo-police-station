    ...
    <footer className="mt-24 border-t border-border bg-surface">
      <div className="mx-auto max-w-6xl px-4 py-14 sm:px-6">
        <div className="grid gap-10 md:grid-cols-[1.4fr_1fr_1fr]">
          <div>
            ...
            <p className="mt-4 max-w-xs text-sm leading-relaxed text-ink-soft">
              서울 5호선 김포·검단 연장(확정)에 김포경찰서역 신설의 필요성을 공공데이터로
              분석하고, 김포 장기동의 교통복지와 이동권을 제안하는 시민 플랫폼입니다.
            </p>
          ...
          <div>
            ...
            <ul className="mt-4 space-y-2.5 text-sm text-ink-soft">
              ...
              <li>국가교통DB(KTDB) · 국가대중교통DB(TAGO)</li>
              <li>경기데이터드림 · 김포시 통계</li>
              <li>국토교통부 · 대도시권광역교통위원회 · 경기도</li>
            </ul>
        ...
        <div className="mt-12 flex flex-col gap-3 border-t border-border pt-6 text-xs text-ink-muted sm:flex-row sm:items-center sm:justify-between">
          <p>© {new Date().getFullYear()} 5호선 김포경찰서역 신설 데이터 플랫폼. 시민 자율 운영 · 비영리.</p>
          <p>김포경찰서역 추진위원회 · 라베니체·한강센트럴자이 등 주민·상인 일동</p>
        </div>
      </div>
    </footer>
  );
